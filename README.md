...
* `jellyfin_generate_self_signed_certificate`: whether to generate a self-signed
  ...
  role will fail to start the reverse proxy HTTP server. It is recommended to use the [autocert bot](https://certbot.eff.org/)
* `jellyfin_certificate_via_letsencrypt_certbot`: whether to have autocert bot handle the certs, defaults to `false`
